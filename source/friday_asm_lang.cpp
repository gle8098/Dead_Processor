...
FRIDAY_INST(in,   0x04, {})            { scanf("%d", BytesAs<int*>(emu->get_stack_ptr())); }
...
FRIDAY_INST(ci2f, 0x2e, {})           { emu->push(AsBytes(static_cast<float>(emu->pop_int())), sizeof(friday_constant_t)); }
// ci2f (full convert float to integer) = pop float && push integer of the same value
FRIDAY_INST(cf2i, 0x2f, {})           { emu->push(AsBytes(static_cast<int>(emu->pop_float())), sizeof(friday_constant_t)); }
//------------------------------------------------------------------------------------
inline void InstDepart(Emulator* emu) {
    emu->push(AsBytes(emu->ip), sizeof(emu->ip));
}
inline void InstUnconditionalJump(Emulator* emu) {
    emu->ip = BytesAs<int32_t>(emu->get_arg_ptr());
}
template <typename U, typename T>
inline void InstConditionalJump(Emulator* emu, T condition) {
